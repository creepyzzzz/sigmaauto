...
DEFAULT_APP_USER_AGENT = "Dart/3.8 (dart:io)"
...
def extract_telegram_key_from_url(url: str) -> str:
    ...
    try:
        ...
        if "_" in start_param:
            return start_param.split("_")[-1].strip()
        return start_param.strip()
    except Exception:
        return None
...
def fetch_key_flow(baseurl: str, verify: bool, debug: bool, user_agent: str = None) -> tuple:
    ...
    """
    session = requests.Session()
    ua = user_agent or DEFAULT_BROWSER_USER_AGENT
            ...
            if second_response.status_code == 200:
                ...
                if base64_match:
                    ...
                    if decrypted_html:
                        ...
                        post_headers = {
                            "User-Agent": session.headers.get("User-Agent", DEFAULT_BROWSER_USER_AGENT),
...
def main():
    ...
    app_user_agent = DEFAULT_APP_USER_AGENT
    ...
    try:
        ...
        if args.direct_url:
            key, associated_url, error = process_direct_url(args.direct_url, verify=verify, debug=debug, user_agent=browser_user_agent)
            if key:
                ok(f"Final key: {key}")
        ...
        if args.default_flow:
            ...
            try:
                headers, resp = get_initial_response_headers(target_url, app_user_agent, verify, debug)
                ...
                key, associated_url, error = fetch_key_flow(baseurl, verify=verify, debug=debug, user_agent=browser_user_agent)
        ...
        key, associated_url, error = process_direct_url(link, verify=verify, debug=debug, user_agent=browser_user_agent)
